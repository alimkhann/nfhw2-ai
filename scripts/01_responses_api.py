...
def create_thread_with_messages(client):
    ...
    thread = client.beta.threads.create(
        messages=[
            {
                "role": "user",
                "content": "Explain the difference between a definite and an indefinite integral."
            },
            {
                "role": "user",
                "content": "What integration types are there for different equations?"
            }
        ]
    )
...
def demonstrate_polling_run(client, assistant_id, thread_id):
    ...
    run = client.beta.threads.runs.create(
        ...
        instructions="Please provide clear, educational explanations suitable for someone learning calculus."
    )
...
def demonstrate_streaming_run(client, assistant_id, thread_id):
    ...
    client.beta.threads.messages.create(
        thread_id=thread_id,
        role="user",
        content="Can you give me a practical example of using calculus in real life?"
    )
    ...
    stream = client.beta.threads.runs.create(
        ...
        assistant_id=assistant_id,
        stream=True,
        instructions="Provide a concise but practical example with a real-life application."
    )
...
def main():
    ...
    print(f"✅ Assistant updated with file search: {assistant_id}")
